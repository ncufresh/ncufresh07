...
if ($curruser->haveperm($curruser->p_handler->getpermvalid()))
{
	...
	else if (isset($_POST["mdreply"]) || isset($_GET["mdreply"]))			//modify article (not ajax)
	{
		...
		if (($curruser->uno != $forumreply->poster_no) && !in_array($curruser->uid, $bm) && !($currmodule->isadmin($curruser)))
			echo "找不到文章";
	...
	else if (isset($_POST["rmreply"]))						// remove reply by post (ajax)
	{
		...
		if ($forumreply->reply_no <= 0)
			echo "找不到文章";
		else if (($curruser->uno != $forumreply->poster_no) && !in_array($curruser->uid, $bm) && !($currmodule->isadmin($curruser)))
			echo "找不到文章";
		else
		{
