...
if ($forumboard->board_no > 0)
{
	...
	echo "<link>".htmlencode($currconfig->url."/module/".$currmodule->name."/viewboard.php?no=".$forumboard->board_no)."</link>\n";
	...
	for ($i = 0;$i < count($forumtopic);$i++)				// modify data from each topic
	{
		...
		echo "<guid isPermaLink=\"true\">".htmlencode($currconfig->url."/module/".$currmodule->name."/viewtopic.php?no=".$forumtopic[$i]->topic_no)."</guid>\n";

		echo "<title>".htmlencode($forumtopic[$i]->title)."</title>\n";

		echo "<link>".htmlencode($currconfig->url."/module/".$currmodule->name."/viewtopic.php?no=".$forumtopic[$i]->topic_no)."</link>\n";
