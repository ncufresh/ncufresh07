...
if ($curruser->haveperm($curruser->p_handler->getpermvalid()))
{
	if (isset($_POST["dopush"]))			// push by post (not ajax)
	{
		...
		else if (!empty($_POST["content"]))
		{
			...
			_redirect($currconfig->url."/module/".$currmodule->name."/viewtopic.php?no=".$forumreply->topic_no);
		}
	}
	else if (isset($_POST["rmpush"]))		// remove push by post (not ajax)
	{
		...
		if ($forumpush->push_no <= 0)
			echo "找不到推文";
		else if (($curruser->uno != $forumpush->poster_no) && !in_array($curruser->uid, $bm) && !($currmodule->isadmin($curruser)))
			echo "找不到推文";
		else
		{
