...
if ($forumtopic->topic_no > 0)
{
	...
	echo "<link>".htmlencode($currconfig->url."/module/".$currmodule->name."/viewtopic.php?no=".$forumtopic->topic_no)."</link>\n";
	...
	for ($i = 0;$i < count($forumreply);$i++)			// modify data from each topic
	{
		...
		echo "<guid isPermaLink=\"true\">".htmlencode($currconfig->url."/module/".$currmodule->name."/viewtopic.php?no=".$forumtopic->topic_no)."</guid>\n";
