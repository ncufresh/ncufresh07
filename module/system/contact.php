...
{
	if (isset($_GET["read_contact"]) && isset($_GET["mno"]))
	{
		...
		$email = ($tmp["isguest"]) ? "mailto:".$tmp["email"] : $currconfig->url."/module/".$currmodule->name."/msgsend.php?fno=".$tmp["sender"];
	...
	else
	{
		...
		$p = multipage($page, $maxpage, $currconfig->phpself);
		$currtpl->assign("p", $p);

		for ($i = 0;$tmp = $currdb->fetch_array($result2);$i++)
		{
			$mno[$i] = $tmp["mno"];
			$email[$i] = ($tmp["isguest"]) ? "mailto:".$tmp["email"] : $currconfig->url."/module/".$currmodule->name."/msgsend.php?fno=".$tmp["sender"];
...
else
{
	if (!empty($_POST["title"]) && !empty($_POST["content"]))
	{
		...
		$currdb->query("INSERT INTO `".$currdb->prefix("contact")."` (mno, sender, email, title, content, time, isguest, ip) VALUES('', '".$sender."', '".$email."', '".$title."', '".$content."', '".$time."', '".$isguest."', '".$ip."')");
